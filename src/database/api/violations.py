...
from database.models import Violation, StudentActivity, Student, Exam, Room, Seat
...
def get_all_violations(
    ...
    """
    Admins and Investigators can view all violations with enriched student/room data.
    """
    ...
    for violation in violations:
        ...
        violation_data = {
            ...
            "evidence_url": violation.evidence_url,
            "student_id": None,
            "student_name": None,
            "seat_number": None,
            "room": None,
        ...
        if activity:
            ...
            if student:
                violation_data["student_id"] = str(student.student_id)
                violation_data["student_name"] = student.name
                
                # Get seat assignment
                seat = db.query(Seat).filter(Seat.student_id == student.student_id).first()
                if seat:
                    violation_data["seat_number"] = seat.seat_number
                    # Get room
                    room = db.query(Room).filter(Room.room_id == seat.room_id).first()
                    if room:
                        violation_data["room"] = room.room_number
            
            # Get exam
            if activity.exam_id:
                exam = db.query(Exam).filter(Exam.exam_id == activity.exam_id).first()
                if exam:
                    violation_data["exam"] = exam.course
        
        enriched_violations.append(ViolationRead(**violation_data))
