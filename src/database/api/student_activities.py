...
from database.models import StudentActivity, Student, Exam, Room, Seat
...
def get_all_student_activities(
    ...
    """
    Admins and Investigators can view all student activities with enriched data.
    """
    if current_user.get("user_type") not in ["admin", "investigator"]:
        raise HTTPException(status_code=403, detail="Access denied")

    activities = db.query(StudentActivity).all()
    enriched_activities = []
    ...
    for activity in activities:
        ...
        student = db.query(Student).filter(Student.student_id == activity.student_id).first()
        if student:
            activity_data["student_name"] = student.name
            
            # Get seat assignment
            seat = db.query(Seat).filter(Seat.student_id == student.student_id).first()
            if seat:
                activity_data["seat_number"] = seat.seat_number
                # Get room
                room = db.query(Room).filter(Room.room_id == seat.room_id).first()
                if room:
                    activity_data["room"] = room.room_number
        
        # Get exam
        if activity.exam_id:
            exam = db.query(Exam).filter(Exam.exam_id == activity.exam_id).first()
            if exam:
                activity_data["exam_name"] = exam.course
        
        enriched_activities.append(StudentActivityRead(**activity_data))
