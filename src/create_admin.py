...
def recreate_database():
    """Drop the existing database if it exists, then recreate it."""
    default_engine = create_engine(
        f"postgresql+psycopg2://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/postgres",
        isolation_level='AUTOCOMMIT'
    )
    ...
    with default_engine.connect() as conn:
        ...
        if exists:
            # Terminate active connections before dropping (required in PostgreSQL)
            conn.execute(text(f"""
...
if __name__ == "__main__":
    ...
    # 2️⃣ Drop and recreate all tables
    Base.metadata.drop_all(bind=engine)
    ...
    # 3️⃣ Create default super admin
    create_super_admin()
